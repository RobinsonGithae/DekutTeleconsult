...
public  static  final int MSG_TYPE_LEFT=0;
    ...
    public DocChatAdapter(Context mContext, List<Chat> mChats, String ImageURL ) {
        ...
        if (mContext.getClass().equals(DoctorChatActivity.class)){
            activity=((DoctorChatActivity)mContext);
        }

    }

    public DocChatAdapter(Activity activity, Context mContext, List<Chat> mChats, String ImageURL ) {
    ...
    @NonNull
    @Override
    public DocChatAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {


        //INITIALIZE view model to perform chat selection
        docchatViewModel= ViewModelProviders.of((FragmentActivity) activity).get(ChatViewModel.class);
    ...
    @Override
    public void onBindViewHolder(@NonNull DocChatAdapter.ViewHolder holder, int position) {
...
//observer method on get text set below
                              ...
                              //=new ChatViewModel();
                      docchatViewModel.getMutableLiveData().observe((LifecycleOwner) activity, new Observer<String>() {
                          @Override
